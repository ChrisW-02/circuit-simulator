...
public:
    ...
    virtual ~Component(){
        std::cout << CompName <<" Base class component destructor." << std::endl;
    }
...
struct Matrix{
    ...
    */
    ...
    void print(){
        for (int i = 0; i < dim; i++){
        for (int j = 0; j < dim; j++){
            std::cout << std::left << std::setw(20) << mat[i][j]<< "\t"; //make the entries align
...
int main(){
    ...
    //if it is ac analysis get the angular frequency conditions
...
////////////////////////////////building class and insert values to the matrix for each frequency
    ...
    element* b;
    b = new element[n]; //right side of equation, currents

    element* x;
    x = new element[n]; //the unknown of the equation, voltages
    ...
    for(int i = 0; i < line.size(); i++){
        ...
        if(firstLetter == 'V'){
            ...
            if(i ==0){
                j--;
                b[j] =V1.V;
                for(int k=0; k<n; k++){ //changing row j to represent vj = -vsrc
                A.mat[j][k] = 0;
            }
                A.mat[j][j] = -1;
            }
                ...
                i--;
                b[i] =V1.V;
                for(int k=0; k<n; k++){ //changing row i to represent vi = vsrc
                A.mat[i][k] = 0;
            }
                A.mat[i][i] = 1;
            }
